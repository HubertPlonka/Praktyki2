...
class IPbusTestClass;
	...
	task write_to_file(string filepath, int number_of_adress); 
		...
		while ( intf.ipb_out.ipb_addr < number_of_adress )begin
		  ...
		  $display("%h %h", intf.ipb_out.ipb_addr, intf.ipb_in.ipb_rdata);
    ...
    task read(input [31:0] addres);
        ...
        $display("%0h", intf.ipb_in.ipb_rdata);
